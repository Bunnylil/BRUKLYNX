document.addEventListener('DOMContentLoaded', () => {
  ...
  signupButton.addEventListener('click', async (e) => {
    ...
    try {
      ...
      const usernameCheckResponse = await fetch(`http://localhost:3000/api/auth/check-username?username=${username}`);
      ...
      const emailCheckResponse = await fetch(`http://localhost:3000/api/auth/check-email?email=${email}`);
      ...
      // If username and email are unique, proceed with Firebase signup
...
googleButton.addEventListener('click', async () => {
  ...
  try {
    ...
    const googleId = user.uid; // Google's unique ID for the user
